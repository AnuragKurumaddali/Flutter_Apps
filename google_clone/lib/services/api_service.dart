...
class ApiService {
  ...
  Future<Map<String, dynamic>> fetchData(
    ...
    try {
      if(!isDummyData) {
        String queryTerm = query.contains(" ") ? query.split(" ").join("%20"): query;
        final response = await http.get(Uri.parse("https://www.googleapis.com/customsearch/v1?key=$apiKey&cx=$contextKey&q=$queryTerm&start=$start'"));
